 ...
 * @param tokens
 */
export function prog(tokens: Token[]): ASTNode {
...
/**
 * new构造函数声明 newCallExpression :: new Identifier '(' primary (',',primary) ')'
 */
function newExpression(ts: TokenStream) {
  let token = ts.getToken();
  if (notToken(token, TType.NEW)) {
    ts.unGetToken(token);
    return null;
  }
  const child = callExpression(ts);
  if (!child) {
    ts.unGetToken(token);
    return null;
  }
  const node = new ASTNode(ASTNodeType.New, token);
  node.addChild(child);
  return node;
}
...
function expressionStatement(ts: TokenStream): ASTNode | null {
  return newExpression(ts) || callExpression(ts) || unaryExpression(ts) || binaryExpression(ts);
}
